...
module.exports = async function sendVerifyMessage(
  guild,
  user,
  channelId,
  messageId,
  ...
  await database.getServerSettings(guild.id, async (serverSettings) => {
    if (
      (channelId !== serverSettings.channelID ||
        messageId !== serverSettings.messageID) &&
      !interaction
    ) {
      return;
    }
    if (!serverSettings.status) {
      await user
        .send(getLocale(serverSettings.language, 'userBotError'))
        .catch(() => {});
      return;
    }
    try {
